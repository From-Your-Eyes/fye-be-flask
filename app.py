...
import random
import string
from azure.communication.email import EmailClient  # assuming this hypothetical library exists
...
openai.api_key = "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"
...
def handle_pdf(file_stream):
    # We read the PDF from the file stream provided
    ...
    for page_num in range(len(pdf.pages)):
        ...
        if not text:
            # No text found, so we convert the PDF page to an image and do OCR
            images = convert_from_path(file_stream, first_page=page_num + 1,
                                       last_page=page_num + 1)  # Convert specific page
            if images:  # Making sure the page was converted
                # Assuming only one image per page
                image = images[0]

                # We'll use the OCR function on the image
                buffered = io.BytesIO()
                image.save(buffered, format="JPEG")
                image_base64 = base64.b64encode(buffered.getvalue()).decode()  # Convert bytes to string

                ocr_result = ocr(image_base64)  # Run your OCR function here
                text_content.append(ocr_result)  # Adding the OCR results
            else:
                # Handle the case where a page could not be converted to an image
                text_content.append("[Error: page could not be converted to image]")
        else:
            # Text found, no need for OCR
            text_content.append(text)

    # Combine all text pieces together
    full_pdf_text = " ".join(text_content)
    return full_pdf_text  # or process the text as you need
...
def get_post_and_ai_comment(post_id, image_index, question):
    try:
        ...
        if result:
            ...
            text_to_generate = f"{ai_comment} {question}"

            # Generate a response using ChatGPT
            response = openai.Completion.create(
                model="gpt-3.5-turbo",  # or your specific engine
                prompt=text_to_generate,  # we're sending the entire prompt constructed from chat messages
                max_tokens=250  # Adjust the number of tokens as needed
            )

            # Extract the AI-generated comment from the response
            ai_generated_comment = response.choices[0].text.strip()

            # Update the aiComment field of the corresponding entity
            if "images" in result[0] and f"d_{image_index}" in result[0]["images"]:
                result[0]["images"][f"d_{image_index}"]["aiComment"] = ai_generated_comment

                # Update the Cosmos DB document with the new aiComment
                container_posts.upsert_item(result[0])

            return ai_generated_comment  # Return the AI-generated comment
        else:
